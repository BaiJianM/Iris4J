...
export default function App() {
  const [light, setLight] = useState(() => {
    try {
      return localStorage.getItem("iris-lite.theme") === "light"
    } catch {
  ...
  useEffect(() => {
    ...
    window.addEventListener("iris-lite.settings-saved", handler)
    return () => window.removeEventListener("iris-lite.settings-saved", handler)
  }, [refreshIdentity])

  // 设置页切主题
  useEffect(() => {
    const handler = (e: Event) => setLight((e as CustomEvent<boolean>).detail)
    window.addEventListener("iris-lite.theme-change", handler)
    return () => window.removeEventListener("iris-lite.theme-change", handler)
  }, [])
      ...
      <header className="flex h-14 shrink-0 items-center gap-4 border-b border-border bg-bg-elevated px-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[16px] font-semibold tracking-tight">iris-lite 控制台</span>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <Button
            ...
            onClick={() => {
              ...
              setLight(next)
              try {
                localStorage.setItem("iris-lite.theme", next ? "light" : "dark")
              } catch {
                /* ignore */
              }
              window.dispatchEvent(new CustomEvent("iris-lite.theme-change", { detail: next }))
            }}
          >
            {light ? "🌙 暗色" : "☀ 亮色"}
          </Button>
