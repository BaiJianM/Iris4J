...
/**
 * 交叉编码器精判端口：对 (query, candidate) 句对输出相关性分数。
 *
 ...
 *
 * <p><b>角色定位</b>：只做精判（precision），不做召回（recall）。
 * 调用方（LlmCacheService）先用 KNN 召回候选（阈值放宽、宁滥勿缺），
 * 再用本端口逐对精判，分数过门限才放行命中。
 *
