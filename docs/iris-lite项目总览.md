...
| LangCache | LLM 响应缓存，省 token | T16 两级命中（精确 promptHash → KNN 0.90）+ T19 数据版本守卫/fresh bypass（超出官方：官方只有 TTL+手动删） |
...
- 官方 LangCache 新鲜性只有 TTL + 按 attributes 手动批量删，**无数据变更联动**——T19 守卫为超出官方的补强；官方「省 90% token」是营销标准（up to），文档公式 = 月输出 token × 命中率；
...
|---|---|---|
| P0-P3 | CDC 查询链路、工具注册、动态 Schema 热载 + tools/list_changed | verify-p3.sh / verify-tools.sh / verify-index.sh |
...
| T19 | 数据版本守卫 + fresh bypass | verify-console-api.sh F 段 |
| T20 | Schema 关系映射编辑 + MCP 描述补关系 | verify-schema-edit.sh |
| T21 | Agent Demo：控制台 Agent 对话（SSE 流式 + 工具循环 + 缓存/守卫联动） | verify-agent-demo.sh |
| T22 | LLM 缓存重排门槛：rerank 召回放宽 + 槽位校验 + 交叉编码器（防微改写误命中） | verify-llm-cache-precision.sh |
...
| T26 | 时区标准定论：Debezium 对 DATETIME 写死 UTC，无法配置；Agent 日界随之锁定 UTC | verify-cdc-completeness.sh（T26/T29 共用最终判定） |
...
> 注：上表所列**多数历史脚本已随 demo 命名空间退役移除（2026-09-09）**，里程碑验收标准保留备查。**现仓内 `deploy/verify/` 实际只有 9 个文件**：`run-final-acceptance.sh`（T26-28 编排）、`verify-cache-invalidation.sh`（T25）、`verify-semantic-cache-index.sh`（T28）、`verify-cdc-completeness.sh`（T26/T29）、`verify-llm-cache-index.sh`（T31）、`monitor-cdc-import.sh`、`stream-stats.lua`、`safe-trim-streams.sh`、`replay-dlq.sh`。
>
> **仓内已不存在、且不可从历史恢复的脚本**（本项目非 git 仓库，无版本历史）：verify-p3 / verify-tools / verify-index / verify-p2 / verify-memory-v0 / verify-agentkeys / verify-console-api / verify-schema-edit / verify-agent-demo / verify-llmcache / verify-llm-cache-precision / verify-extractor / verify-embedder。仅 `verify-related.sh` 可从 `deploy/backups/demo-namespace-retired-20260909/` 恢复。**故 §4 表格的「验收脚本」列与 §7 引用的 verify-tools.sh §7 均属历史标准，不能作为可执行依据**；重跑需先按 ecomm 实体重写（见 §9 待办）。
...
- Schema 编辑、agent key 管理等管理面端点须 legacy operator 身份；鉴权关闭（未配 `iris.security.api-key`）时全部允许访问（本地开发姿态）。
...
**多路召回 → RRF 融合 → rerank**，生效面 = 长期记忆检索（HYBRID 模式，缺省）+ LLM 缓存语义查找：
...
3. **时效词旁路（2026-09-13）**：`FreshnessDetector`（`iris.cache.freshness-bypass.*`，缺省开，词表 最新/现在/目前/当前/实时/此刻/今天/今日/刚刚/刚才 可配）——问题含时效指示词 = 语义锚定说话时刻，缓存条目（历史快照）不可信：LLM 缓存 lookup 必经点强制 miss（reason=fresh-keyword，REST/MCP/Agent 三入口全覆盖）；Agent 侧经 ToolContext→QueryRequest.fresh 穿透语义/精确两层查询缓存跳过读强制现算（回写保留，快照对后续普通查询仍有效）。与 T19 守卫互补：守卫管「数据变了」，本机制管「问题要求此刻标准」。
3. **融合**：RRF `score=Σ 1/(k+rank)`（`RrfFusion`，k=60，只认排名不认分数标准）；候选池上限 `iris.rag.candidates`（12）。
4. **重排**：`CrossEncoderReranker.scoreBatch` 批量判定（正向前向 1 次 + 反向逐对 = N+1 次往返）；LLM 缓存保留守卫→槽位→rerank 门序，记忆按 rerank 分重排（失败保持 RRF 序，不不可用）。
...
2. **~~预聚合 `iris.preagg.*`~~（已移除，2026-09-14）**：曾落地 CDC 增量 HINCRBY 计数 + `PreAggregatedEntityQueryService` 直读装饰器（链 4→5 层）。移除理由：收益面窄（只覆盖显式声明的单维度、换维度即失效）、CDC 写路径常驻成本不值得。教训积累：加速类派生数据必须先算清「查询形状覆盖率」，覆盖率低就不配常驻。
...
验收（历史）：MCP aggregate_entity 直读 2957 组与基准准 FT.AGGREGATE 逐值一致（preagg 验证记录，组件已删）。
...
- CDC 积压标准 = XINFO GROUPS 的 **lag**（Debezium 消费不删事件，XLEN 是历史累计只增不减）。
...
- **双塔与交叉编码器对数字/日期槽位差异均不敏感**（T22 实测，当时为本地 bge 模型）：事故句对 cos 0.916、交叉编码器 rerank 0.9998，与真同义对（rerank 0.999-1.0）完全重叠，无阈值可分——槽位一致性校验（数字序列比对）必须保留在重排链路里，且先于 rerank 执行。
...
- 验收脚本 ecomm 化（**缺口比原先记录的大**）：2026-09-10 全量核验发现，文档/配置共引用 17 个脚本，**其中 9 个在仓内已不存在**——verify-p3 / verify-tools / verify-index / verify-p2 / verify-memory-v0 / verify-agentkeys / verify-console-api / verify-schema-edit / verify-agent-demo / verify-llmcache / verify-llm-cache-precision 均已随 demo 命名空间退役被删，且**本项目不是 git 仓库，无历史可恢复**；仅 verify-related.sh 可从 `deploy/backups/demo-namespace-retired-20260909/` 恢复。
  - 优先补 **verify-llm-cache-precision.sh**：T22 重排门槛 `threshold: 0.50` 的唯一标定依据就是它，脚本丢失意味着该阈值**当前无法复核**。重建需按 ecomm 实体造基准 query 对（同义改写 / 微改写 / 事故原案三类），判据沿用原标准（同义 3/3 命中、微改写 4/4 拒绝、事故原案 slotRejected≥1）。
  - 其余脚本按里程碑价值排序重写（P3/T14/T18/T20/T21/T15 各自的标准在各里程碑行有描述）。
