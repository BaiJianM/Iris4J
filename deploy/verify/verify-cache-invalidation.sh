...
# 前置：iris-lite 已在 $IRIS_BASE 运行；iris-redis / iris-mysql 容器可用。
#
...
if curl -s --noproxy '*' -m 5 "$BASE/actuator/health" | grep -q '"status":"UP"'; then
  ok "iris-lite 健康（$BASE）"
else
  bad "iris-lite 未就绪（$BASE/actuator/health）"
  echo "验收中止：请先启动应用"
  exit 1
fi
...
if [ "${XREAD_CONNS:-0}" -gt 200 ]; then
  bad "检测到多个 iris-lite 实例（XREADGROUP 连接=${XREAD_CONNS}，单实例应 ≈135）"
  echo "原因：每条事件会被两个实例各投影一遍（双写），且任一实例跑着旧字节码时，"
  echo "      会同时污染「数据条数」与「scan 增量」两项结论。"
  echo "验收中止：请只保留一个实例（jps -l 可列出全部 iris-lite 进程）后重试"
  exit 1
fi
